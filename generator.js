...
const newTemplate = (req, res, next) => {
  ...
  const configPath = {
    model: [
      ...
      `./database/mongodb/models/${templateName}.js`,
    ],
    controller: [
...
const copyFile = (params) => {
  ...
  fs.copyFile(origPath, destFile, (err) => {
    ...
    console.log(`${clr.fg.cyan}GT: 📝 ${origPath} is copied`);
    console.log(`${clr.fg.cyan}GT: 📝 ${destFile} is pasted`);
    readFile(params);
  });
};
...
const copyFolder = (params) => {
  ...
  fsx.copy(origPath, destPath, (err) => {
    ...
    console.log(`${clr.fg.cyan}GT: 📝 ${origPath} is copied`);
    console.log(`${clr.fg.cyan}GT: 📝 ${destPath} is pasted`);
    readFile(params);
  });
};
...
const readFile = (params) => {
  ...
  fs.readFile(destFile, "utf8", (err, data) => {
    ...
    console.log(`${clr.fg.cyan}GT: 📝 ${destFile} is read`);
    // replace with regex
...
const writeFile = ({ destFile, content }) => {
  fs.writeFile(
    ...
    (err) => {
      ...
      console.log(`${clr.fg.cyan}GT: 📝 ${destFile} is saved`);
    }
  );
};
...
const replaceFile = (params) => {
  ...
  fs.readFile(origPath, "utf8", (err, data) => {
    if (err) throw err;
    // file is successfully read
    console.log(`${clr.fg.cyan}GT: 📝 ${origPath} is read`);
    // replace with regex
    const content = data.replace(regexStr, (matched) => mapObj[matched]);
...
const appendFile = ({ destFile, content }) => {
  fs.appendFile(
    ...
    (err) => {
      ...
      console.log(`${clr.fg.cyan}GT: 📝 ${destFile} is appended`);
    }
  );
};
